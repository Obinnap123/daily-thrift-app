...
export async function inviteAdminAction(input: { name: string; email: string }) {
  ...
  let delivery: Awaited<ReturnType<typeof sendAgentInvitationEmail>>;
  try {
    delivery = await sendAgentInvitationEmail({
      to: result.data.email!, agentName: result.data.name, invitationToken: invitation.token,
      applicationOrigin: await getApplicationOrigin(), role: "Admin",
...
export async function resendAdminInvitationAction(adminId: string) {
  ...
  const invitation = createStaffVerificationToken();
  const result = await prisma.$transaction(async (tx) => {
    const admin = await tx.user.findFirst({ where: { id: adminId, role: "ADMIN", isActive: true, emailVerifiedAt: null }, select: { id: true, name: true, email: true } });
    if (!admin?.email) return fail("No pending Admin invitation was found.");
    await tx.staffEmailVerificationToken.upsert({
      where: { userId: admin.id },
      create: { userId: admin.id, tokenHash: invitation.tokenHash, expiresAt: invitation.expiresAt },
...
export async function setAdminActiveAction(adminId: string, isActive: boolean) {
  ...
  const result = await prisma.$transaction(async (tx) => {
    await tx.$queryRaw`SELECT pg_advisory_xact_lock(392541, 1)::text`;
    const admin = await tx.user.findFirst({ where: { id: adminId, role: "ADMIN" }, select: { id: true, isActive: true } });
    if (!admin) return fail("Admin account not found.");
    if (isActive && !admin.isActive) {
      const other = await tx.user.findFirst({ where: { role: "ADMIN", isActive: true, id: { not: adminId } }, select: { id: true } });
      if (other) return fail("Another Admin is active. Deactivate that account first.");
    }
