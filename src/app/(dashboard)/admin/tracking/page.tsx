...
export default async function AdminTrackingPage() {
  ...
  return <div className="flex min-h-screen flex-col"><DashboardHeader title="Customer Tracking" /><DashboardNav links={links} /><main className="flex-1 space-y-4 p-4 sm:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">31-Day Tracking — All Customers</h2><p className="text-sm text-gray-500">The latest monthly sheet for every open savings period. Select a customer to see all previous sheets.</p></div><QuickPayButton customers={options} isAdmin label="Quick Pay" /></div><Card className="overflow-hidden p-0"><AllCustomerTrackingTable rows={rows} role="ADMIN" /></Card><p className="flex flex-wrap gap-3 text-xs text-ink-muted"><span className="text-brand-ink">✓ Paid</span><span className="text-danger">! Unfunded past day</span><span>— Pending or unavailable</span></p></main></div>;
}
