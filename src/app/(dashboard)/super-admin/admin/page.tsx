...
export default async function SuperAdminAccountsPage() {
  ...
  const records = await prisma.user.findMany({ where: { role: "ADMIN" }, orderBy: { createdAt: "desc" }, select: { id: true, name: true, email: true, isActive: true, emailVerifiedAt: true, lastLoginAt: true, archivedAt: true } });
  const admins = records.map((admin) => ({
    ...
    lastLoginLabel: admin.lastLoginAt ? formatBusinessDateTime(admin.lastLoginAt) : null,
    archived: Boolean(admin.archivedAt),
  }));
